...
class Solution3:
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        Note:
            ...
            - return and array containing product at i except i itself
        Intuition:
            - prefix product from left and right
                - iterate from left to right on nums
                    - calculate left prefix product excluding nums[i]
                        - if i == 0:
                            - initialize left product as 1
                - initialize rProduct to 1
                - iterate from right to left on nums:
                    - update lProduct[i] *= rProduct
                - return the lProduct array
        """

        # TC: O(n) / SC: O(n), O(1) if not counting output
        lProduct = []  # SC: O(n), O(1) if not counting the output as extra space
        lprod = 1
        for i, n in enumerate(nums):  # TC: O(n)
            lProduct.append(lprod)
            lprod *= nums[i]
